...
- `↑ ↓ Σ CH` —— 累计输入 / 输出 / 总量（新增输入+缓存读+缓存写+输出）/ 缓存命中率
...
- `5h: W: ⏱` —— 套餐剩余（MiniMax / GLM / Kimi / DeepSeek / OpenCode Go / Command Code 内置套餐，需在 /stats 里为当前 provider 启用）
  - DeepSeek 为余额制（`¥12.3`，非 5h/W 百分比），会整条原样显示，不受上面的子项开关过滤
...
- 完成后：会话末尾插入一条汇总，完成时刻（24 小时制系统时间）独占第一行，第二行为总耗时与耗时分解（模型流式〔含思考〕，紧随其后为该 run 的平均首 token 延迟 TTFT；再接 工具 / 其他），第三行为本次任务的 token 指标（输入/输出/总量/缓存命中率/平均速率，对齐 footer 风格）：

  ```
  [2026-08-26 10:20:12]
  总耗时：01:23  模型 00:45 TTFT 1.2s · 工具 00:30 · 其他 00:08
  ↑1.2k ↓345 Σ52.5k CH80% ⚡12.3 t/s
  ```

  TTFT 为**平均值**：每轮从 `turn_start` 到首个流式事件（text/thinking/toolcall 任一）的耗时取平均；无样本（如直接报错、无输出）时该项不显示。不足 1 分钟保留 1 位小数（如 `1.2s`）。

  经 `appendEntry` 持久化、不进入 LLM 上下文，`/resume` 后仍在；升级前的旧条目只显示前两行且无耗时分解。
